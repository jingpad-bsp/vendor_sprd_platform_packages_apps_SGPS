...
/**
 * This class represents the current state of a GPS satellite. This class is used in conjunction
 * with the {@linkGpsStatus} class.
 */
final class GpsSatellite implements Comparable<GpsSatellite> {
    ...
    /**
     * Used by {@linkgetGpsStatus} to copy LocationManager's cached GpsStatus
     * instance to the client's copy.
     */
    ...
    /**
     * Returns true if the satellite was used by the GPS engine when calculating the most recent GPS
     * fix.
     * 
    ...
    public int compareTo(GpsSatellite another) {
        if (mPrn > another.mPrn) {
            return 1;
        } else if (mPrn < another.mPrn) {
            return -1;
        } else {
            return 0;
        }
